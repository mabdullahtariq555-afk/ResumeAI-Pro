...
function dashboard() {
  ...
  <main class="app">
    <div class="topbar"><div><h1>Resume Dashboard</h1></div></div>
...
function builder(){
  ...
  <main class="app">
    ...
    <div class="builder"><div class="panel">
      <div class="section"><h3>Resume Import</h3><div class="field"><label>Upload Resume</label><input id="resumeUpload" type="file" accept=".txt,.pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,text/plain"><small class="muted">TXT, PDF, DOCX - maximum 5 MB</small></div><div class="field"><label>Extracted / Raw Resume Text</label><textarea id="rawResume" rows="8" placeholder="Paste resume text or upload a file...">${esc(r.rawText||'')}</textarea></div><div class="field"><label>Job Description</label><textarea id="jdInput" rows="7" placeholder="Paste the job description here...">${esc(r.jd||'')}</textarea></div><div class="actions" style="margin-top:10px"><button class="btn" onclick="uploadResume()">Import File</button><button class="btn primary" onclick="runAnalysis()">Analyze Resume</button></div></div>
...
async function runJDMatch(){
  ...
  if(!jd){
    return toast('Paste a job description first.');
  }
  ...
  try{
    const resumeText = [
      buildPlainText(current),
      current.rawText || ''
    ].filter(Boolean).join('\n\n');

    const prompt = `Compare this resume with this job description.
...
${jd}`;
    ...
    const raw=await callAI(prompt,'application/json');
    const j=cleanJSON(raw);

    const matchScore=Math.max(
      0,
      Math.min(100,Number(j.matchScore)||0)
    );
    ...
    out.innerHTML=`
      ...
      <b>Matching skills</b>
      <p>${(j.matchingSkills||[]).map(esc).join(' - ')||'None identified'}</p>

      <b>Missing requirements</b>
      <p>${(j.missingKeywords||[]).map(esc).join(' - ')||'None identified'}</p>

      <b>Improvements</b>
      <ul>
        ${(j.improvements||[]).map(x=>`<li>${esc(x)}</li>`).join('')}
      </ul>
    `;
  }catch(e){
    out.textContent='AI: '+e.message;
  }
}
